...
$CFG->dblibrary = env('CFG_DBLIBRARY', 'native');
$CFG->dbhost    = env('CFG_DBHOST', env('POSTGRES_HOST', 'db'));
$CFG->dbname    = env('CFG_DBNAME', env('POSTGRES_DATABASE', 'postgres'));
$CFG->dbuser    = env('CFG_DBUSER', env('POSTGRES_USER', 'postgres'));
$CFG->dbpass    = env('CFG_DBPASS', env('POSTGRES_PASSWORD', 'postgres'));
...
$CFG->wwwroot = env('CFG_WWWROOT', "http://localhost");
...
$CFG->dataroot  = env('CFG_DATAROOT', '/var/www/moodledata');
...
$CFG->directorypermissions = env_as_int('CFG_DIRECTORYPERMISSIONS', 02775);
...
//
// Seconds for files to remain in caches. Decrease this if you are worried about students being served outdated versions of uploaded files.
$CFG->filelifetime = env_as_int('CFG_FILELIFETIME', 60*60*6);
...
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\database') {
  $CFG->session_handler_class = '\core\session\database';
  $CFG->session_database_acquire_lock_timeout = env_as_int('CFG_SESSION_DATABASE_ACQUIRE_LOCK_TIMEOUT', 120);
}

//   File session handler (file system locking required):
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\file') {
  $CFG->session_handler_class = '\core\session\file';
  $CFG->session_file_save_path = env('CFG_SESSION_FILE_SAVE_PATH', $CFG->dataroot.'/sessions');
}

//   Memcached session handler (requires memcached server and extension):
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\memcached') {
  $CFG->session_handler_class = '\core\session\memcached';
  $CFG->session_memcached_save_path = env('CFG_SESSION_MEMCACHED_SAVE_PATH', 'cache:11211');
  $CFG->session_memcached_prefix = env('CFG_SESSION_MEMCACHED_PREFIX', 'memc.sess.key.');
  $CFG->session_memcached_acquire_lock_timeout = env_as_int('CFG_SESSION_MEMCACHED_ACQUIRE_LOCK_TIMEOUT', 120);
  $CFG->session_memcached_lock_expire = env_as_int('CFG_SESSION_MEMCACHED_LOCK_EXPIRE', 7200); // Ignored if PECL memcached is below version 2.2.0
  $CFG->session_memcached_lock_retry_sleep = env_as_int('CFG_SESSION_MEMCACHED_LOCK_RETRY_SLEEP', 150); // Spin-lock retry sleeptime (msec). Only effective for tuning php-memcached 3.0.x (PHP 7)
}

//   Redis session handler (requires redis server and redis extension):
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\redis') {
  $CFG->session_handler_class = '\core\session\redis';
  $CFG->session_redis_host = env('CFG_SESSION_REDIS_HOST', 'cache');
  $CFG->session_redis_port = env_as_int('CFG_SESSION_REDIS_PORT', 6379);  // Optional.
  $CFG->session_redis_database = env_as_int('CFG_SESSION_REDIS_DATABASE', 1);  // Optional, default is db 0.
  $CFG->session_redis_auth = env('CFG_SESSION_REDIS_AUTH', ''); // Optional, default is don't set one.
  $CFG->session_redis_prefix = env('CFG_SESSION_REDIS_PREFIX', 'session_'); // Optional, default is don't set one.
  $CFG->session_redis_acquire_lock_timeout = env_as_int('CFG_SESSION_REDIS_ACQUIRE_LOCK_TIMEOUT', 120);
...
$CFG->reverseproxy = env_as_bool('CFG_REVERSEPROXY', false);

// Enable when using external SSL appliance for performance reasons.
// Please note that site may be accessible via http: or https:, but not both!
$CFG->sslproxy = env_as_bool('CFG_SSLPROXY', false);
...
$CFG->headerloguser = env_as_int('CFG_HEADERLOGUSER', 0);
...
if (!empty(env('CFG_EMAILCONNECTIONERRORSTO'))) {
  $CFG->emailconnectionerrorsto = env('CFG_EMAILCONNECTIONERRORSTO');
}
...
if (env('CFG_ENV', 'local') == 'local' && env_as_bool('CFG_DEBUG', false)) {
  // NOT FOR PRODUCTION SERVERS!

  // Force a debugging mode regardless the settings in the site administration
  // 24565 = (E_ALL & ~E_NOTICE & ~E_DEPRECATED & ~E_WARNING)
  $CFG->debug = (E_ALL & ~E_DEPRECATED & ~E_WARNING);
  $CFG->debugdisplay = env_as_int('CFG_DEBUGDISPLAY', 1);
  @error_reporting($CFG->debug);
  @ini_set('display_errors', $CFG->debugdisplay);
  // You can specify a comma separated list of user ids that that always see debug messages, this overrides the debug flag in $CFG->debug and $CFG->debugdisplay for these users only.
  $CFG->debugusers = env('CFG_DEBUGUSERS', '1');

  // Prevent theme caching
  $CFG->themedesignermode = env_as_bool('CFG_THEMEDESIGNERMODE', true); // NOT FOR PRODUCTION SERVERS!

  // Enable verbose debug information during fetching of email messages from IMAP server.
  $CFG->debugimap = env_as_bool('CFG_DEBUGIMAP', true);
  ...
  $CFG->debugsmtp = env_as_bool('CFG_DEBUGSMTP', true);

  // Prevent JS caching
  $CFG->cachejs = env_as_bool('CFG_CACHEJS', false); // NOT FOR PRODUCTION SERVERS!

  // Prevent Template caching
  $CFG->cachetemplates = env_as_bool('CFG_CACHETEMPLATES', false); // NOT FOR PRODUCTION SERVERS!

  // Restrict which YUI logging statements are shown in the browser console. For details see the upstream documentation:
  ...
  $CFG->yuiloginclude = ['moodle-course-categoryexpander' => env_as_bool('CFG_YUILOGINCLUDE', true),];
  $CFG->yuilogexclude = ['moodle-core-notification' => env_as_bool('CFG_YUILOGEXCLUDE', true),];

  // Set the minimum log level for YUI logging statements. For details see the upstream documentation:
  //   http://yuilibrary.com/yui/docs/api/classes/config.html#property_logLevel
  $CFG->yuiloglevel = env('CFG_YUILOGLEVEL', 'debug');

  // Prevent core_string_manager application caching
  $CFG->langstringcache = env_as_bool('CFG_LANGSTRINGCACHE', false); // NOT FOR PRODUCTION SERVERS!

  // When working with production data on test servers, no emails or other messages should ever be send to real users
  $CFG->noemailever = env_as_bool('CFG_NOEMAILEVER', true);    // NOT FOR PRODUCTION SERVERS!

  // Divert all outgoing emails to this address to test and debug emailing features
  $CFG->divertallemailsto = env('CFG_DIVERTALLEMAILSTO', 'root@localhost.local'); // NOT FOR PRODUCTION SERVERS!

  // Except for certain email addresses you want to let through for testing. Accepts a comma separated list of regexes.
  $CFG->divertallemailsexcept = env('CFG_DIVERTALLEMAILSEXCEPT', 'tester@dev.com, fred(\+.*)?@example.com'); // NOT FOR PRODUCTION SERVERS!

  // Uncomment if you want to allow empty comments when modifying install.xml files.
  $CFG->xmldbdisablecommentchecking = env_as_bool('CFG_XMLDBDISABLECOMMENTCHECKING', true);    // NOT FOR PRODUCTION SERVERS!

  // Since 2.0 sql queries are not shown during upgrade by default. Please note that this setting may produce very long upgrade page on large sites.
  $CFG->upgradeshowsql = env_as_bool('CFG_UPGRADESHOWSQL', false); // NOT FOR PRODUCTION SERVERS!

  // Add SQL queries to the output of cron, just before their execution
  $CFG->showcronsql = env_as_bool('CFG_SHOWCRONSQL', false);

  // Force developer level debug and add debug info to the output of cron
  $CFG->showcrondebugging = env_as_bool('CFG_SHOWCRONDEBUGGING', true);

  // Force result of checks used to determine whether a site is considered "public" or not (such as for site registration).
  $CFG->site_is_public = env_as_bool('CFG_SITE_IS_PUBLIC', false);
} else {
  // You can specify a comma separated list of user ids that that always see
  // debug messages, this overrides the debug flag in $CFG->debug and $CFG->debugdisplay
  // for these users only.
  $CFG->debugusers = env('CFG_DEBUGUSERS', null);

  // Enable verbose debug information during fetching of email messages from IMAP server.
  $CFG->debugimap = env_as_bool('CFG_DEBUGIMAP', false);

  // Enable verbose debug information during sending of email messages to SMTP server.
  // Note: also requires $CFG->debug set to DEBUG_DEVELOPER.
  $CFG->debugsmtp = env_as_bool('CFG_DEBUGSMTP', false);

  // Restrict which YUI logging statements are shown in the browser console.
  // For details see the upstream documentation:
  //   http://yuilibrary.com/yui/docs/api/classes/config.html#property_logInclude
  //   http://yuilibrary.com/yui/docs/api/classes/config.html#property_logExclude
  $CFG->yuiloginclude = ['moodle-course-categoryexpander' => env_as_bool('CFG_YUILOGINCLUDE', false),];
  $CFG->yuilogexclude = ['moodle-core-notification' => env_as_bool('CFG_YUILOGEXCLUDE', false),];

  // Set the minimum log level for YUI logging statements.
  // For details see the upstream documentation:
  //   http://yuilibrary.com/yui/docs/api/classes/config.html#property_logLevel
  $CFG->yuiloglevel = env('CFG_YUILOGLEVEL', 'error');

  // Add SQL queries to the output of cron, just before their execution
  $CFG->showcronsql = env_as_bool('CFG_SHOWCRONSQL', false);

  // Force developer level debug and add debug info to the output of cron
  $CFG->showcrondebugging = env_as_bool('CFG_SHOWCRONDEBUGGING', false);

  // Force result of checks used to determine whether a site is considered "public" or not (such as for site registration).
  $CFG->site_is_public = env_as_bool('CFG_SITE_IS_PUBLIC', true);
}
